  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
        <div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.45rem', letterSpacing: '0.18em', color: '#333', marginBottom: 3 }}>{tool.label}</div>
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.42rem', color: '#333', marginTop: 4, textAlign: 'right', letterSpacing: '0.1em' }}>
        {tool.level}%
      </div>
    </motion.div>
  )
    ...
    >
      ...
      <div style={{ paddingBottom: isLast ? 0 : '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.6rem', marginBottom: 4 }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5rem', letterSpacing: '0.15em', color: isLast ? ACCENT_BRIGHT : ACCENT }}>{item.year}</span>
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.95rem', color: '#e0e0e0', letterSpacing: '-0.01em' }}>{item.title}</span>
        </div>
        <p style={{ fontFamily: 'var(--font-sans)', fontSize: '0.78rem', color: '#444', lineHeight: 1.6, margin: 0 }}>{item.desc}</p>
      </div>
    </motion.div>
  )
}
  ...
  return (
    <div style={{ background: BASE, minHeight: '100vh', color: '#e0e0e0' }}>
      ...
      <div style={{
        ...
        <Link href="/" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', letterSpacing: '0.2em', color: '#555', textDecoration: 'none' }}>
          ← РОГОВ
        </Link>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5rem', letterSpacing: '0.2em', color: '#222' }}>ОБО МНЕ</div>
        <button
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}
            style={{ fontFamily: 'var(--font-mono)', fontSize: '0.5rem', letterSpacing: '0.2em', color: ACCENT, marginBottom: '1rem' }}>
            00 / ИДЕНТИЧНОСТЬ
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 1.2 }}
            ...
            {[['ЛОКАЦИЯ', 'Россия'], ['СТАТУС', 'Открыт для проектов'], ['СТЕК', 'Figma · Adobe · AI · 3D']].map(([label, val]) => (
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #1a1a1a', padding: '0.7rem 0' }}>
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.47rem', letterSpacing: '0.15em', color: '#333' }}>{label}</span>
                <span style={{ fontFamily: 'var(--font-sans)', fontSize: '0.72rem', color: '#777' }}>{val}</span>
              </div>
            ))}
          </motion.div>
        </div>
          ...
          {STATS.map((s, i) => {
            ...
            return (
              <motion.div key={s.label} ref={ref}
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.45rem', letterSpacing: '0.18em', color: '#333', marginTop: 8 }}>{s.label}</div>
              </motion.div>
            )
          })}
        </div>
      ...
      <section style={{ padding: '3rem 1.25rem', borderBottom: '1px solid #111' }}>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.47rem', letterSpacing: '0.18em', color: '#222', marginBottom: '1.5rem' }}>01 / КТО Я</div>
        {[
          ...
          return (
            <motion.p key={i} ref={ref}
              ...
              style={{ fontFamily: 'var(--font-sans)', fontSize: '0.9rem', color: i === 0 ? '#999' : '#555', lineHeight: 1.75, marginBottom: '1rem', margin: '0 0 1rem' }}>
              {text}
            </motion.p>
          )
        })}
      ...
      <section style={{ padding: '3rem 1.25rem', borderBottom: '1px solid #111' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '1.5rem' }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.45rem', letterSpacing: '0.18em', color: '#222' }}>02 / ИНСТРУМЕНТЫ</span>
        </div>
      ...
      <section style={{ padding: '3rem 1.25rem', borderBottom: '1px solid #111' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '1.5rem' }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.45rem', letterSpacing: '0.18em', color: '#222' }}>03 / ИСТОРИЯ</span>
        </div>
        <div>
